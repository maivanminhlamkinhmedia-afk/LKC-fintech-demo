...
async function login(browser: Browser, clipboard = false) {
  ...
  if (clipboard) await test.step('CLIP_PERMISSION', async () => {
...
async function assertSafePastedDom(page: Page, phase: 'paste' | 'reload') {
  const editor = body(page)
  await test.step(phase === 'paste' ? 'CLIP_DOM_TEXT' : 'CLIP_RELOAD_DOM_TEXT', async () => {
    await expect(editor).toContainText('Nội dung dán tiếng Việt')
  })
  await test.step(phase === 'paste' ? 'CLIP_LINK_COUNT' : 'CLIP_RELOAD_LINK_COUNT', async () => {
    await expect(editor.locator('a')).toHaveCount(1)
  })
  const link = editor.getByRole('link', { name: 'Liên kết an toàn', exact: true })
  await test.step(phase === 'paste' ? 'CLIP_LINK_HREF' : 'CLIP_RELOAD_LINK_HREF', async () => {
    await expect(link).toHaveAttribute('href', 'https://example.com/vi')
  })
  await test.step(phase === 'paste' ? 'CLIP_LINK_TARGET' : 'CLIP_RELOAD_LINK_TARGET', async () => {
    await expect(link).toHaveAttribute('target', '_blank')
  })
  await test.step(phase === 'paste' ? 'CLIP_LINK_REL' : 'CLIP_RELOAD_LINK_REL', async () => {
    await expect(link).toHaveAttribute('rel', 'noopener noreferrer nofollow')
  })
  await test.step(phase === 'paste' ? 'CLIP_LINK_CLASS' : 'CLIP_RELOAD_LINK_CLASS', async () => {
    await expect(link).not.toHaveAttribute('class')
  })
  await test.step(phase === 'paste' ? 'CLIP_LINK_TITLE' : 'CLIP_RELOAD_LINK_TITLE', async () => {
    await expect(link).not.toHaveAttribute('title')
  })
  await test.step(phase === 'paste' ? 'CLIP_DANGEROUS_ELEMENTS' : 'CLIP_RELOAD_DANGEROUS_ELEMENTS', async () => {
    await expect(editor.locator('script, iframe, object, embed, img, svg')).toHaveCount(0)
  })
  await test.step(phase === 'paste' ? 'CLIP_EVENT_ATTRIBUTES' : 'CLIP_RELOAD_EVENT_ATTRIBUTES', async () => {
  ...
  await test.step(phase === 'paste' ? 'CLIP_SCRIPT_EXECUTION' : 'CLIP_RELOAD_SCRIPT_EXECUTION', async () => {
...
test('EDIT-18 real HTML clipboard paste removes unsafe content and persists canonical safe links', async ({ browser }) => {
  const { page } = await test.step('CLIP_LOGIN', async () => login(browser, true))
  await test.step('CLIP_INPUT', async () => { await newDraft(page, 'paste') })
  await test.step('CLIP_WRITE', async () => {
  ...
  await test.step('CLIP_NATIVE_PASTE', async () => {
    await body(page).focus()
    await page.keyboard.press('ControlOrMeta+V')
  })
  await assertSafePastedDom(page, 'paste')
  const id = await test.step('CLIP_SAVE_NAVIGATE', async () => {
    await save(page)
    return await registerCreated(page)
  })
  const validated = await test.step('CLIP_DB_CANONICAL', async () => {
  ...
  await test.step('CLIP_DB_LINKS', async () => {
  ...
  await test.step('CLIP_RELOAD', async () => { await page.reload() })
  await assertSafePastedDom(page, 'reload')
})
